...
def get_valid_polys_my(polys):
    polys_ = []
    for poly in polys:
        exterior = Polygon(poly).exterior
        poly_ = np.array(list(exterior.coords),dtype=np.float64)
        polys_.append(poly_)
    return polys_

def filter_tiny_polys(polys, cls_ids, min_area=5):
    """
    filter tiny polys
    """
    polys_ = []
    ...
    for idx in range(len(polys)):
        ...
        if x_max - x_min >= 1 and y_max - y_min >= 1 and Polygon(poly).area > min_area:
            polys_.append(poly)
            cls_ids_.append(cls_id)
    return polys_, cls_ids_


def get_cw_polys(polys):
    """
    obtain clockwise polys
    """
    polys2 = []
    for i in range(len(polys)):
        poly = polys[i]
...
def filter_outside_polys(polys, ids, bound_size=(128,128),th=0):
    h_bound,w_bound = bound_size
    polys_ = []
    ids_ = []
    for idx in range(len(polys)):
        poly = polys[idx]
        ...
        w_min = poly[:, 0].min()
        h_min = poly[:, 1].min()
        if (h_max-h_bound) > th or (w_max-w_bound) > th or h_min < 0 or w_min < 0:
            continue
        else:
            polys_.append(poly)
            ids_.append(ids[idx])
...
def cutoff_outside_polys(polys, ids, bound_size=(128,128),th=0):
    h_bound,w_bound = bound_size
    polys_ = []
    ids_ = []
    for idx in range(len(polys)):
        ...
        w_min = poly[:, 0].min()
        h_min = poly[:, 1].min()
        if w_min > (w_bound-1) or w_max < 0 or  h_min > (h_bound-1) or h_max < 0:
            continue
        else:
            poly[:,0] = np.clip(poly[:, 0],a_min=0,a_max=w_bound-1)
            poly[:,1] = np.clip(poly[:, 1],a_min=0,a_max=h_bound-1)
            polys_.append(poly)
            ids_.append(ids[idx])
    return polys_, ids_
...
def get_init(box):
    if snake_config.init == 'quadrangle': # get the mid points of each box edge
...
def uniformsample(pgtnp_px2, newpnum):
    ...
    pgtnext_px2 = pgtnp_px2[idxnext_p] #shift edge to one index
    edgelen_p = np.sqrt(np.sum((pgtnext_px2 - pgtnp_px2) ** 2, axis=1)) # calc the edge len
    ...
    # we simply add it uniformly
        ...
        edgenum = np.round(edgelen_p * newpnum / np.sum(edgelen_p+0.00001)).astype(np.int32)
        for i in range(pnum):
            if edgenum[i] == 0:
                edgenum[i] = 1
        ...
        psample = []
        for i in range(pnum):
            pb_1x2 = pgtnp_px2[i:i + 1] #point_begin_1x2
            pe_1x2 = pgtnext_px2[i:i + 1] #point_end_1x2

            pnewnum = edgenum[i]
            wnp_kx1 = np.arange(edgenum[i], dtype=np.float32).reshape(-1, 1) / edgenum[i] #weight_n_point_kx1

            pmids = pb_1x2 * (1 - wnp_kx1) + pe_1x2 * wnp_kx1 #bilinear interpolation
