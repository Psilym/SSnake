...
def make_dist_data_loader(cfg, is_train=True, is_distributed=False, max_iter=-1, required_sampler=False):
    if is_train:
        batch_size = cfg.train.batch_size
        shuffle = True
        drop_last = False
    ...
    transforms = make_transforms(cfg, is_train)
    dataset = make_dataset(cfg, dataset_name, transforms, is_train)
    sampler = dist.DistributedSampler(dataset, shuffle=shuffle)
    if is_train:
        batch_sampler = make_batch_data_sampler(cfg, sampler, batch_size, drop_last, max_iter)
    else:
        batch_sampler = make_batch_data_sampler(cfg, sampler, batch_size, drop_last, max_iter)
